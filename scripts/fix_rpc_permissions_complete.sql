-- COMPLETE FIX for RPC function permissions
-- This fixes the 42501 forbidden errors by:
-- 1. Granting service_role execute permission on all RPC functions
-- 2. Ensuring authenticated can still execute
-- Run this in Supabase SQL Editor

-- ============================================
-- STEP 1: Grant service_role execute on all critical RPC functions
-- ============================================
...
-- ============================================
-- STEP 2: Ensure authenticated can still execute (for client-side calls)
-- ============================================
...
-- ============================================
-- STEP 3: Fix get_summary_min_composite security check
-- Make it more robust to detect service_role
-- ============================================
...
DECLARE
  ...
  v_budget jsonb;
BEGIN
  -- SECURITY: More robust service_role detection
  -- When using service_role key, the JWT claim might be in different format
  -- Check multiple possible claim locations
  BEGIN
  ...
  -- Detect service_role: 
  -- 1. Explicit role claim = 'service_role'
  -- 2. No role claim AND no sub claim (service_role context)
  -- 3. Check if we can access other users' data (service_role privilege)
  v_is_service_role := (
    v_claim_role = 'service_role' OR
    (v_claim_role IS NULL AND v_claim_sub IS NULL)
  );

  -- Allow service_role to access any user's data
  -- For authenticated users, enforce p_user_id matches JWT subject
  IF NOT v_is_service_role THEN
    IF v_claim_sub IS NULL OR v_claim_sub <> p_user_id THEN
      RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501', MESSAGE = 'Access denied: user_id mismatch or invalid authentication';
    END IF;
  END IF;
...
-- Re-grant permissions after function replacement
GRANT EXECUTE ON FUNCTION public.get_summary_min_composite(uuid, integer, date, date) TO service_role;
GRANT EXECUTE ON FUNCTION public.get_summary_min_composite(uuid, integer, date, date) TO authenticated;
