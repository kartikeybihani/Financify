...
export async function sendPushNotification(
  ...
  try {
    ...
    console.log(
      `[NOTIFICATION_SENDER] Sending to token: ${expoPushToken.substring(0, 20)}...`,
    );
    console.log(
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        error: `Expo API error: ${response.status} - ${errorText.substring(0, 100)}`,
      };
    }
    ...
    if (Array.isArray(result.data)) {
      ...
      return {
        success: false,
        error: `Invalid response format from Expo API: ${JSON.stringify(result).substring(0, 200)}`,
      };
    }
