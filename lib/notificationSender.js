...
export async function getPushTokensForUser(userId) {
  ...
  try {
    ...
    console.error(
      "[NOTIFICATION_SENDER] Error in getPushTokensForUser:",
      error
    );
    return [];
  }
...
export function generateNotificationContent(trigger) {
  ...
  switch (triggerType) {
    ...
    case "custom":
      if (metadata.pattern_type === "biggest_mover") {
        title = "📈 Biggest mover today";
        body =
          metadata.pattern_description ||
          "Your biggest holding moved today. Tap to see.";
        // Navigate to investments section in insights tab when tapped
        data.navigation_target = "insights_investments";
      } else {
        title = "Finny here!";
        body =
...
export async function sendPushNotification(
  ...
  try {
    ...
    // Log payload for debugging (mask token)
    console.log("Expo URL:", EXPO_PUSH_API_URL);
    console.log(
      `[NOTIFICATION_SENDER] Sending to token: ${expoPushToken.substring(
        0,
        20
      )}...`
    );
    console.log(
      `[NOTIFICATION_SENDER] Payload:`,
      JSON.stringify(
        { ...payload, to: `${expoPushToken.substring(0, 20)}...` },
        null,
        2
      )
    );
    ...
    if (!response.ok) {
      ...
      console.error(
        `[NOTIFICATION_SENDER] Expo API error (${response.status}):`,
        errorText
      );
      return {
        success: false,
        error: `Expo API error: ${response.status} - ${errorText.substring(
          0,
          100
        )}`,
      };
    ...
    console.log("Expo raw response:", result);

    // Expo Push API v2 returns different formats:
    // Single token: { data: { status: 'ok', id: '...' } }
    // Multiple tokens: { data: [{ status: 'ok', id: '...' }, ...] }
    let receipt;
    if (Array.isArray(result.data)) {
      ...
      if (result.data.length === 0) {
        return {
          success: false,
          error: "Empty response array from Expo API",
        };
      }
      ...
      // Unexpected format
      console.error(
        `[NOTIFICATION_SENDER] Unexpected Expo API response format:`,
        JSON.stringify(result, null, 2)
      );
      return {
        success: false,
        error: `Invalid response format from Expo API: ${JSON.stringify(
          result
        ).substring(0, 200)}`,
      };
    }
    ...
    if (receipt.status === "ok") {
      ...
      const errorMsg =
        receipt.message || receipt.details?.error || "Unknown error";

      // Log the full receipt for debugging
      console.error(
        `[NOTIFICATION_SENDER] Expo API receipt error:`,
    ...
    console.error(
      "[NOTIFICATION_SENDER] Error sending push notification:",
      error
    );
...
async function markTokenInactive(expoPushToken) {
  try {
    ...
    if (error) {
      ...
      console.log(`[NOTIFICATION_SENDER] Marked invalid token as inactive`);
    }
  } catch (error) {
    console.error(`[NOTIFICATION_SENDER] Error in markTokenInactive:`, error);
  }
}
...
async function handleFailedTrigger(trigger) {
  try {
    ...
    console.error(`[NOTIFICATION_SENDER] Error in handleFailedTrigger:`, error);
  }
}
