...
async function backfillRecurringCategories(supabase, userId, recurringRows) {
  // Fetch stream_ids that user has dismissed - we must NOT re-apply recurring to their transactions
  const streamIdsInBatch = recurringRows.map((r) => r.stream_id).filter(Boolean);
  let dismissedStreamIds = new Set();
  if (streamIdsInBatch.length > 0) {
    const { data: dismissed } = await supabase
      .from("recurring_streams")
      .select("stream_id")
      .eq("user_id", userId)
      .eq("user_dismissed", true)
      .in("stream_id", streamIdsInBatch);
    dismissedStreamIds = new Set((dismissed || []).map((d) => d.stream_id));
  }

  const transactionToStreamMap = new Map();
  recurringRows.forEach((stream) => {
    if (dismissedStreamIds.has(stream.stream_id)) return;
...
export async function refreshAndStoreRecurringForItem({
  ...
  if (recurringRows.length > 0) {
    // Do NOT include user_dismissed in payload - it will be preserved on conflict (existing rows keep their value)
    ...
    // Ensure user_dismissed streams stay hidden (is_active could be overwritten by Plaid data on upsert)
    await supabase
      .from("recurring_streams")
