...
async function loadUserMemory(userId, query = null) {
  ...
  try {
    ...
    // If no query provided, use a generic query to get recent memories
    ...
    // Transform Supermemory results to our format
    const memories = supermemoryResults.map((result) => {
      // Extract content from various possible fields
      const content = result.memory || result.summary || result.content || "";
      const summary = result.summary || result.memory || "";

      // Extract metadata
      const metadata = result.metadata || {};

      // Get document ID (for updates/deletes if needed)
      const documentId = result.documents?.[0]?.id || result.id;

      return {
        id: result.id || documentId,
        documentId: documentId,
        content: content,
        summary: summary, // AI-generated summary from Supermemory
        metadata: metadata,
    ...
    console.log(
      `🧠 [MEMORY] Loaded ${result.totalCount} relevant memories for user ${userId}`
    );
    return result;
  } catch (error) {
...
async function searchSupermemoryMemories(userId, query, options = {}) {
  ...
  try {
    // Build metadata filters for relevance
    // Prefer high financial relevance memories, but don't exclude others
    const filters = {
      AND: [
        // Prefer high relevance, but include medium/low if no high relevance found
        {
          OR: [
            { key: "financial_relevance", value: "high", negate: false },
            { key: "financial_relevance", value: "medium", negate: false },
            { key: "financial_relevance", value: "low", negate: false },
          ],
        },
      ],
    };
