...
async function retrieveFeedbackPatterns(userId, currentTopic = null) {
  ...
  try {
    ...
    if (feedbackMemories.length > 0) {
      console.log(
        `🔍 [FEEDBACK_PATTERNS] Sample memory metadata:`,
        JSON.stringify(feedbackMemories[0].metadata || {}, null, 2)
      );
    }

    // Filter for feedback memories specifically
    const feedbackOnly = feedbackMemories.filter(
      (m) =>
        m.metadata?.memory_type === "message_feedback" ||
        (Array.isArray(m.metadata?.tags) &&
          (m.metadata.tags.includes("response_preference") ||
            m.metadata.tags.includes("style_learning")))
    );
    ...
    // Fallback: If no feedback memories found with semantic search,
    // try a broader search to ensure we catch all feedback memories
    let finalFeedbackOnly = feedbackOnly;
    if (feedbackOnly.length === 0) {
      console.log(
        `🔄 [FEEDBACK_PATTERNS] No feedback found with semantic search, trying broader search...`
      );
      const broaderSearch = await searchSupermemoryMemories(userId, "*", {
        limit: 100,
        threshold: 0.0, // Get all memories
      });

      finalFeedbackOnly = broaderSearch.filter(
        (m) =>
          m.metadata?.memory_type === "message_feedback" ||
          (Array.isArray(m.metadata?.tags) &&
            (m.metadata.tags.includes("response_preference") ||
      ...
      console.log(
        `🔄 [FEEDBACK_PATTERNS] Broader search found ${finalFeedbackOnly.length} feedback memories`
      );
    }
    ...
    const topicPreferences = Object.entries(patterns.topics)
      ...
      .map(([topic, counts]) => ({
        topic,
        preference: counts.positive > counts.negative ? "prefers" : "dislikes",
...
function buildFeedbackContext(feedbackData) {
  if (!feedbackData || feedbackData.preferences.length === 0) {
    return null;
  }
  ...
  if (feedbackData.deepInsights.length > 0) {
