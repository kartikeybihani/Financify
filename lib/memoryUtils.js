...
async function retrieveFeedbackPatterns(userId, currentTopic = null) {
  ...
  try {
    ...
    const feedbackMemories = await searchSupermemoryMemories(userId, query, {
      limit: 15, // Get more feedback memories for pattern detection
      threshold: 0.35, // Lower threshold to catch more feedback patterns
    });
    ...
    console.log(
      `✅ [FEEDBACK_PATTERNS] Retrieved ${feedbackOnly.length} feedback memories, extracted ${preferences.length} preferences, ${highConfidenceInsights.length} deep insights`
    );
    ...
    return {
      ...
      feedbackCount: feedbackOnly.length,
    };
  } catch (error) {
    console.error(
