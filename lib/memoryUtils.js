...
function getRecentConversationTurns(
  ...
  const entry = conversationTurnsCache.get(key);
  if (!entry) {
    console.log(`⚠️ [GET_TURNS] No cache entry found for key: ${key}`);
    return [];
  }
  
  const age = Date.now() - entry.timestamp;
  if (age > CONVERSATION_TURNS_TTL) {
    console.log(`⚠️ [GET_TURNS] Cache entry expired (age: ${Math.round(age / 1000)}s, TTL: ${Math.round(CONVERSATION_TURNS_TTL / 1000)}s) for key: ${key}`);
  ...
  if (rawTurns.length === 0) {
    console.log(`⚠️ [GET_TURNS] Cache entry has no turns for key: ${key}`);
    return [];
  }
  ...
  console.log(`✅ [GET_TURNS] Returning ${kept.length} turns (from ${rawTurns.length} total) for key: ${key}`);
  return kept.reverse();
}
