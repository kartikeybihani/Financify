 ...
 * @param {object} messageMetadata - Additional metadata (messageType, hasActions, hasGoalOffer, finny_style, etc.)
...
async function storeMessageFeedback(
  ...
  if (feedbackType === "positive") {
    contentParts.push(
      `User liked Finny's response about ${characteristics.topics.join(", ")}.`
    );
  } else {
    contentParts.push(
      `User disliked Finny's response about ${characteristics.topics.join(
        ", "
      )}.`
    );
    if (reportText) {
      contentParts.push(`User reported: "${reportText}"`);
    }
  }
  contentParts.push(`User's question was: "${userMessage}"`);
  contentParts.push(
    `Finny's response was: "${finnyResponse.substring(0, 500)}${
