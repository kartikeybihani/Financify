...
function appendConversationTurns(userId, chatId, userText, assistantText) {
  ...
  if (assistantText && assistantText.trim().startsWith("Classification:")) {
    console.log(
      "⚠️ [APPEND_TURNS] Skipping classification result - not a conversation turn",
    );
    return;
  }
  ...
  if (
    userText &&
    userText.trim() &&
    !userText.trim().startsWith("Classification:")
  ) {
    turns.push({
      role: "user",
  ...
  if (
    assistantText &&
    assistantText.trim() &&
    !assistantText.trim().startsWith("Classification:")
  ) {
    turns.push({
      role: "assistant",
  ...
  console.log(
    `✅ [APPEND_TURNS] Stored ${turns.length} total turns for key: ${key}`,
  );
}
...
async function getRecentConversationTurns(
  ...
  if (entry) {
    ...
    if (age <= CONVERSATION_TURNS_TTL) {
      ...
      if (rawTurns.length > 0) {
        ...
        console.log(
          `✅ [GET_TURNS] Returning ${kept.length} turns from cache (from ${rawTurns.length} total) for key: ${key}`,
        );
        return kept.reverse();
      }
    }
  ...
  try {
    ...
    if (!logs || logs.length === 0) {
      console.log(
        `⚠️ [GET_TURNS] No conversation logs found in database for user: ${userId}`,
      );
      return [];
    }
    ...
    for (const log of logs.reverse()) {
      ...
      if (
        log.user_message &&
        log.user_message.trim() &&
        log.finny_response &&
        log.finny_response.trim() &&
        !log.finny_response.trim().startsWith("Classification:")
      ) {
        // Add user message
        turns.push({
    ...
    console.log(
      `✅ [GET_TURNS] Returning ${kept.length} turns from database (from ${logs.length} logs) for key: ${key}`,
    );
    return kept; // Already in chronological order (oldest to newest)
  } catch (error) {
    console.log(
      `⚠️ [GET_TURNS] Error fetching from database: ${error.message}`,
    );
    return [];
  }
}
  ...
  setInterval(
    () => {
      ...
      for (const [key, entry] of conversationTurnsCache.entries()) {
        if (
          !entry?.timestamp ||
          now - entry.timestamp > CONVERSATION_TURNS_TTL
        ) {
...
const SUPERMEMORY_STORE_TIMEOUT_MS = Math.max(
  Number(process.env.SUPERMEMORY_STORE_TIMEOUT_MS || 12000),
  1000,
); // Write timeout for background document storage (does not block chat response)
...
async function storeSupermemoryDocumentWithRetry(
  ...
  metadata,
  {
    ...
    timeoutMs = SUPERMEMORY_STORE_TIMEOUT_MS,
  } = {},
) {
...
export async function storeGoalDeletionMemory(
  ...
  contentParts.push(`- Target Amount: ${formattedAmount}`);
  contentParts.push(
    `- Current Progress: ${formattedCurrent} (${progressPercent}%)`,
  );
  contentParts.push(`- Target Date: ${goalData.target_date}`);
  contentParts.push(`- Category: ${goalData.category}`);
  if (goalData.status) {
      ...
      const response = await fetchWithTimeout(
        `${SUPERMEMORY_BASE_URL}/v4/search`,
        {
          ...
          body: JSON.stringify({
            ...
            rewriteQuery: false, // Allow query rewriting for better results
            include: {
              documents: true,
...
function deriveNegativeFeedbackInsights(reportText, characteristics = {}) {
  ...
  if (
    ...
    insights.push(
      "User is highly sensitive to inaccurate or invented details.",
    );
  }
  ...
  if (
    /confusing|unclear|hard to understand|doesn't make sense/.test(reportLower)
  ) {
...
async function retrieveFeedbackPatterns(userId, currentTopic = null) {
  ...
  try {
    ...
    safeReports.forEach((report) => {
      ...
      updateTopicPatternCounts(patterns, characteristics.topics, isPositive);
