...
// Track active prebuild_context requests to suppress memory storage warnings
const activePrebuildRequests = new Set(); // Set of userIds with active prebuild_context requests

export function setPrebuildContextActive(userId) {
  if (userId) {
    activePrebuildRequests.add(userId);
  }
}

export function clearPrebuildContextActive(userId) {
  if (userId) {
    activePrebuildRequests.delete(userId);
  }
}

function isPrebuildContextActive(userId) {
  return userId ? activePrebuildRequests.has(userId) : false;
}
...
function getRecentConversationTurns(
  ...
  const entry = conversationTurnsCache.get(key);
  if (!entry) return [];
  if (Date.now() - entry.timestamp > CONVERSATION_TURNS_TTL) {
  ...
  // Enforce a total char budget by dropping oldest messages first.
  // (Per-message trimming already reduces worst-case size.)
  let total = 0;
  const kept = [];
...
async function fetchWithTimeout(
  ...
  console.log(`🚀 [FETCH_TIMEOUT] Starting request [${requestId}]`, {
    method,
    url,
    timeoutMs,
  ...
  try {
    ...
    console.log(`✅ [FETCH_TIMEOUT] Request completed [${requestId}]`, {
      method,
      url,
...
async function storeConversationMemory(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // This ensures fresh data is fetched when user opens memories screen
      ...
      if (isLastAttempt || !isRetryable) {
        ...
        // Suppress warnings during prebuild_context requests
        if (isPrebuildContextActive(userId)) {
          // Silently fail during prebuild_context - don't log warnings
          return null;
        }
      ...
      // Suppress retry warnings during prebuild_context requests
      if (isPrebuildContextActive(userId)) {
        // Silently retry during prebuild_context - don't log warnings
...
async function loadUserProfile(userId) {
  ...
  try {
    ...
    const { data: profile, error } = await supabase
      .from("profiles")
      .select(
        "first_name, last_name, age, occupation, location, finny_style, intent_q1, intent_q2, intent_q3"
      )
      .eq("id", userId)
      .maybeSingle();
...
async function fetchSupermemoryMemories(userId) {
  ...
  try {
    const requestBody = {
      ...
      limit: 10, // Get up to 100 memories
      threshold: 0.3, // Low threshold to get all results
      rerank: false,
      rewriteQuery: false,
      include: {
        documents: true,
    ...
    const response = await fetchWithTimeout(
      `${SUPERMEMORY_BASE_URL}/v4/search`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${SUPERMEMORY_API_KEY.substring(0, 10)}...`,
          "Content-Type": "application/json",
        },
        body: JSON.stringify(requestBody),
      },
...
async function deleteSupermemoryMemory(memoryId, userId) {
  ...
  try {
    ...
    // Body contains: containerTag
    // Note: memoryId may be required in body, query param, or URL path - adjust if needed
    const requestBody = {
      containerTag: `user_${userId}`,
      memoryId: memoryId, // Include memoryId in body to identify which memory to delete
    };
...
async function updateSupermemoryMemory(memoryId, updateData, userId) {
  ...
  try {
    ...
    // Body contains: containerTag and newContent
    // Note: memoryId may be required in body, query param, or URL path - adjust if needed
    const requestBody = {
      containerTag: `user_${userId}`,
      newContent: updateData.content,
      memoryId: memoryId, // Include memoryId in body to identify which memory to update
    };
...
async function retrieveFeedbackPatterns(userId, currentTopic = null) {
  ...
  try {
    ...
    // Feedback memories may not be semantically similar to the query text (they can be terse, meta, or
    // stored with sparse content). Keep the threshold low here, then filter by metadata below.
