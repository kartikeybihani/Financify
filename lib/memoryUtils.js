...
async function loadUserProfile(userId) {
  ...
  if (cached) {
    console.log(`👤 [PROFILE] Using cached profile data for user: ${userId} - finny_style: ${cached.finny_style || 'conversational'}`);
    return cached;
  }
  ...
  try {
    ...
    const finnyStyle = profile?.finny_style && 
    ...
    console.log(`👤 [PROFILE] Loaded profile for user ${userId} - finny_style: ${finnyStyle}${profile?.finny_style !== finnyStyle ? ` (normalized from: ${profile?.finny_style || 'null'})` : ''}`);
    return result;
  } catch (error) {
...
async function retrieveFeedbackPatterns(userId, currentTopic = null) {
  ...
  try {
    ...
    console.log(
      ...
      // Enhanced logging - show content preview only
      if (feedbackMemories.length > 0) {
        console.log(`🔍 [FEEDBACK_PATTERNS] Sample memory content previews:`);
        feedbackMemories
          .slice(0, Math.min(5, feedbackMemories.length))
          .forEach((mem, idx) => {
            ...
            const feedbackType = metadata.feedback_type || "N/A";
            const contentPreview = content.substring(0, 150);
            
            // Log what's unknown if metadata is missing
            if (memoryType === "unknown") {
              console.log(
                `  [${idx + 1}] ⚠️ UNKNOWN - metadata missing or not in expected format. Content preview: ${contentPreview}${content.length > 150 ? '...' : ''}`
              );
              // Log the actual structure to help debug
              console.log(
                `      📋 Memory structure: has metadata=${!!mem.metadata}, metadata keys=${mem.metadata ? Object.keys(mem.metadata).join(", ") : "none"}, has documents=${Array.isArray(mem.documents) && mem.documents.length > 0}, document metadata=${Array.isArray(mem.documents) && mem.documents[0]?.metadata ? Object.keys(mem.documents[0].metadata).join(", ") : "none"}`
              );
            } else {
              console.log(
                `  [${idx + 1}] ${memoryType} | ${feedbackType} | ${contentPreview}${content.length > 150 ? '...' : ''}`
              );
            }
          });
      }
    ...
    // Log the actual preferences being used
    if (preferences.length > 0) {
      console.log(`📋 [FEEDBACK_PATTERNS] Extracted preferences:`);
      preferences.forEach((pref, idx) => {
        console.log(`  ${idx + 1}. ${pref}`);
      });
    }
    
    // Log deep insights if any
    if (highConfidenceInsights.length > 0) {
      console.log(`💡 [FEEDBACK_PATTERNS] Deep insights:`);
