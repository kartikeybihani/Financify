...
function mergeSessionState(userId, partial) {
  ...
  const entry = sessionStateCache.get(userId);
  const current = entry && Date.now() - entry.timestamp <= SESSION_STATE_TTL
    ? entry.state || {}
    : {};
...
async function invalidateSupermemoryDocumentsCache(userId) {
  ...
  try {
    ...
    if (error) {
      console.warn(`⚠️ [CACHE] Failed to invalidate cache for user ${userId}:`, error.message);
    } else {
      console.log(`✅ [CACHE] Invalidated supermemory documents cache for user ${userId}`);
    }
  } catch (error) {
...
async function cacheSupermemoryDocuments(userId, memories) {
  ...
  try {
    const documentsToUpsert = memories.map((memory) => {
      ...
      return {
        ...
        updated_at: memory.updatedAt || document.updatedAt || new Date().toISOString(),
        created_at: memory.createdAt || document.createdAt || new Date().toISOString(),
    ...
    if (error) {
      console.error(`❌ [CACHE] Failed to cache documents for user ${userId}:`, error.message);
    } else {
      console.log(`✅ [CACHE] Cached ${documentsToUpsert.length} documents for user ${userId}`);
    }
  } catch (error) {
...
async function getCachedSupermemoryDocuments(userId) {
  ...
  try {
    ...
    console.log(`✅ [CACHE] Retrieved ${memories.length} cached documents for user ${userId}`);
    return memories;
  } catch (error) {
...
async function deleteCachedSupermemoryDocument(userId, memoryId) {
  ...
  try {
    ...
    if (error) {
      console.warn(`⚠️ [CACHE] Failed to delete cached document:`, error.message);
    } else {
      console.log(`✅ [CACHE] Deleted cached document ${memoryId} for user ${userId}`);
    }
  } catch (error) {
...
async function updateCachedSupermemoryDocument(userId, memoryId, updates) {
  ...
  try {
    ...
    if (error) {
      console.warn(`⚠️ [CACHE] Failed to update cached document:`, error.message);
    } else {
      console.log(`✅ [CACHE] Updated cached document ${memoryId} for user ${userId}`);
    }
  } catch (error) {
