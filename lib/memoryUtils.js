...
async function searchSupermemoryMemories(userId, query, options = {}) {
  ...
  try {
    ...
    // Log full structure of first result to debug metadata retrieval
    if (memories.length > 0) {
      console.log(
        `🔍 [SUPERMEMORY] Sample search result structure:`,
        JSON.stringify(
          {
            hasId: !!memories[0].id,
            hasMemory: !!memories[0].memory,
            hasContent: !!memories[0].content,
            hasMetadata: !!memories[0].metadata,
            metadataType: typeof memories[0].metadata,
            metadataKeys: memories[0].metadata
              ? Object.keys(memories[0].metadata)
              : [],
            hasDocuments: !!memories[0].documents,
            documentsType: Array.isArray(memories[0].documents)
              ? "array"
              : typeof memories[0].documents,
            documentsLength: Array.isArray(memories[0].documents)
              ? memories[0].documents.length
              : 0,
            fullKeys: Object.keys(memories[0]),
          },
          null,
          2
        )
      ...
      // Check if metadata is in documents
      if (
        Array.isArray(memories[0].documents) &&
        memories[0].documents.length > 0
      ) {
        console.log(
          `🔍 [SUPERMEMORY] First document structure:`,
          JSON.stringify(
            {
              hasMetadata: !!memories[0].documents[0].metadata,
              metadataKeys: memories[0].documents[0].metadata
                ? Object.keys(memories[0].documents[0].metadata)
                : [],
              documentKeys: Object.keys(memories[0].documents[0]),
            },
            null,
            2
          )
...
async function storeMessageFeedback(
  ...
  if (feedbackType === "positive") {
    ...
    if (reportText) {
      ...
      // Add feedback-focused line
      contentParts.push(
        `Feedback focus: ${userIdentifier} expressed concern about "${reportText}" - this indicates what should be avoided or improved in future responses from Finny.`
      );
    }
...
async function retrieveFeedbackPatterns(userId, currentTopic = null) {
  ...
  try {
    ...
    console.log(
      `🔍 [FEEDBACK_PATTERNS] Search returned ${feedbackMemories.length} memories before filtering`
    );

    // Enhanced logging to understand memory structure
    if (feedbackMemories.length > 0) {
      // Log first 3 memories in detail
      const sampleMemories = feedbackMemories.slice(
        0,
        Math.min(3, feedbackMemories.length)
      );
      sampleMemories.forEach((mem, idx) => {
        ...
        // Check if metadata is in documents array
        if (!metadata || Object.keys(metadata).length === 0) {
          if (Array.isArray(mem.documents) && mem.documents.length > 0) {
            metadata = mem.documents[0].metadata || {};
            console.log(
              `🔍 [FEEDBACK_PATTERNS] Memory ${
                idx + 1
              } - metadata found in documents array`
            );
          }
        }

        // Log full memory object structure for debugging
        console.log(
          `🔍 [FEEDBACK_PATTERNS] Memory ${idx + 1} full structure:`,
          JSON.stringify(
            {
              topLevelKeys: Object.keys(mem),
              hasMetadata: !!mem.metadata,
              hasDocuments: !!mem.documents,
              metadataDirect: mem.metadata,
              metadataFromDocuments:
                Array.isArray(mem.documents) && mem.documents[0]?.metadata,
              metadataKeys: Object.keys(metadata),
              memory_type: metadata.memory_type,
              feedback_type: metadata.feedback_type,
              tags: metadata.tags,
              hasTags: Array.isArray(metadata.tags),
              contentPreview: content.substring(0, 100) + "...",
              contentIncludesLiked: content.toLowerCase().includes("liked"),
              contentIncludesDisliked: content
                .toLowerCase()
                .includes("disliked"),
              contentIncludesFeedback:
    ...
    console.log(
      `🔍 [FEEDBACK_PATTERNS] After filtering: ${feedbackOnly.length} feedback memories`
    );

    // Log details of filtered memories
    if (feedbackOnly.length > 0) {
      console.log(
        `🔍 [FEEDBACK_PATTERNS] Filtered memory details:`,
        feedbackOnly.map((mem, idx) => ({
          index: idx + 1,
          memory_type: mem.metadata?.memory_type,
          feedback_type: mem.metadata?.feedback_type,
          tags: mem.metadata?.tags,
          contentPreview: (mem.memory || mem.content || "").substring(0, 150),
        }))
      );
    } else if (feedbackMemories.length > 0) {
      console.log(
        `⚠️ [FEEDBACK_PATTERNS] No feedback memories found after filtering. Sample unfiltered memory:`,
        {
          metadata: feedbackMemories[0].metadata,
          contentPreview: (
            feedbackMemories[0].memory ||
            feedbackMemories[0].content ||
            ""
          ).substring(0, 200),
        }
      );
    ...
    console.log(
      `✅ [FEEDBACK_PATTERNS] Retrieved ${finalFeedbackOnly.length} feedback memories, extracted ${preferences.length} preferences, ${highConfidenceInsights.length} deep insights`
    );
