 ...
 * Used by scripts/analyze-recurring-transactions.js and api/analyze-recurring.js
 */
...
export function enrichWithTransactionIds(parsed, transactions) {
  ...
  const isZelle = (merchant) =>
    ZELLE_MERCHANTS.some((z) =>
      (merchant || "").toLowerCase().includes(z),
    );
  ...
  return {
    ...parsed,
    newly_found_subscriptions: enrich(
      parsed.newly_found_subscriptions,
      { requireMatch: true },
    ),
...
export async function upsertFinnyStreamsIntoRecurring(supabase, userId, enriched, existingPlaidMerchants) {
  ...
  const toRow = (item, category, streamType) => {
    ...
    const transactionIds = Array.isArray(item.transaction_ids) ? item.transaction_ids : [];
 ...
 */
export async function runRecurringAnalysis(supabase, userId, itemId, triggerSource = "manual") {
  ...
  const prompt = buildRecurringAnalysisPrompt(
    transactions,
    existingStreams.map((s) => ({ description: s.description, merchant_name: s.merchant_name })),
  );
