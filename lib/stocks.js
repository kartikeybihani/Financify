...
async function fetchStockSnapshot(ticker) {
  ...
  if (quote) {
    console.log(`   Quote: ✅ Current: $${quote.c || "N/A"}, Change: ${quote.d || "N/A"} (${quote.dp || "N/A"}%), High: $${quote.h || "N/A"}, Low: $${quote.l || "N/A"}`);
  } else {
    console.log(`   Quote: ❌ No quote data`);
  }
  if (profile) {
    console.log(`   Profile: ✅ Name: ${profile.name || "N/A"}, Industry: ${profile.finnhubIndustry || "N/A"}, Market Cap: $${profile.marketCapitalization?.toLocaleString() || "N/A"}`);
  } else {
  ...
  if (recs && Array.isArray(recs) && recs.length > 0) {
    ...
    console.log(`   Recommendations: ✅ Buy: ${(latest.strongBuy || 0) + (latest.buy || 0)}, Hold: ${latest.hold || 0}, Sell: ${(latest.sell || 0) + (latest.strongSell || 0)} (Total: ${total})`);
  } else {
  ...
  if (metrics?.metric) {
    ...
    console.log(`   Metrics: ✅ P/E: ${pe ? pe.toFixed(2) : "N/A"}, P/S: ${ps ? ps.toFixed(2) : "N/A"}`);
  } else {
