...
export function buildContextAwarePromptDetailed(
  ...
  const strategies = selectStrategies(
    ...
    conflictResolution,
    decisionRisk,
    advisoryRuntime?.advisory_job || null,
  );
  ...
  const coreIdentity = PROMPT_MODULES.core_identity(advisoryRuntime);
  ...
  const communicationStyle = PROMPT_MODULES.communication_style(
    style,
    advisoryRuntime,
  );
