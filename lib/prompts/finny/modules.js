...
- Provide complete answers that fully address the question, but keep them concise and well-formatted with hyphen lists, bullet characters (•), or numbered lists when needed`;
    }
...
- If resolution.question_policy = none, you may add one short non-question nudge tied to the same topic (specific, not generic).
...
- no generic warnings
- no generic closers (e.g., "consider your budget" / "align with your priorities")
...
- Do not repeat the same number in prose and list items.
- If a natural sentence works better than a list, prefer the sentence.
...
- Use short lists (3-5 items) when needed; no long generic lists.
- Do not end with generic filler questions.

FORMATTING:
- Plain text only.
- Use hyphens (-), bullet characters (•), or numbered lists (1., 2., 3.); never "*" or "+" bullets.
