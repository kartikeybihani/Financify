...
export function synthesizeFinancialData(financialData, intent = null) {
  ...
  const liquidAssets = Number(base.liquidAssets) || 0;
  const totalLiabilities = Number(base.totalLiabilities) || 0;
  const netWorth = Number(base.netWorth) || 0;
  const investmentsTotal = Number(base.investmentsTotal) || 0;
  ...
  console.log(
    `   - Spend: ${financialData.spend ? "present" : "missing"}`,
  );
  console.log(
    `   - CategoryDetails: ${
...
- Total Liabilities (debt): $${totalLiabilities.toLocaleString(undefined, {
  ...
  if (Array.isArray(accounts) && accounts.length > 0) {
    const accountLines = accounts.map((acc) => {
      ...
      const limit = Number(acc.balances?.limit ?? 0) || 0;
      ...
      if (limit > 0 && (type === "credit" || subtype === "credit card")) {
        const utilization = ((Math.abs(balance) / limit) * 100).toFixed(1);
...
${transactionLines.join("\n")}`);
  ...
  if (
    ...
    const categoryLines = spendByCategoryCurrentMonth.map((cat) => {
      ...
      const total = Number(cat.total_spend || cat.amount) || 0;
  ...
  if (financialData.invest) {
    ...
    if (Array.isArray(invest.holdings) && invest.holdings.length > 0) {
      const holdingLines = invest.holdings.map((holding) => {
        ...
        const units = Number(holding.units || holding.quantity) || 0;
        const value = Number(holding.market_value || holding.value) || 0;
        return `- ${symbol}${
...
${holdingLines.join("\n")}`);
    ...
    if (Array.isArray(invest.balances) && invest.balances.length > 0) {
      const balanceLines = invest.balances.map((bal) => {
        ...
        const balance = Number(bal.balance || bal.current_balance) || 0;
...
${balanceLines.join("\n")}`);
  ...
  if (
    ...
    const goalLines = financialData.goals.goals.map((goal) => {
      ...
      const progress = Number(goal.progress_pct) || 
    ...
    if (
      ...
      if (
        ...
        if (catDetails.total_sent !== undefined) {
          breakdownLines.push(
            `Sent: $${Number(catDetails.total_sent || 0).toLocaleString(undefined, {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
        ...
        if (catDetails.total_received !== undefined) {
          breakdownLines.push(
            `Received: $${Number(catDetails.total_received || 0).toLocaleString(undefined, {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
        ...
        sectionContent += `\n\nTotal: $${Math.abs(
          Number(catDetails.total_spend) || 0,
      ...
      if (
        ...
        const monthlyLines = catDetails.monthlyBreakdown.map((month) => {
          ...
          const spend = Number(month.total_spend) || 0;
