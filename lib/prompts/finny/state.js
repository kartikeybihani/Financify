...
export function selectStrategies(
  ...
  const advisoryStrategies = {
    lookup: "factual_strategy",
    understand: "educational_strategy",
    assess: "judgment_strategy",
    improve: "diagnosis_strategy",
    plan: "ordered_plan_strategy",
    crisis_override: "crisis_action_strategy",
  };
  ...
  if (advisoryJob && advisoryStrategies[advisoryJob]) {
    strategies.push({
      name: advisoryStrategies[advisoryJob],
      priority: 1,
      source: "advisory_job",
    });
  } else if (intent?.intent_type && intentStrategies[intent.intent_type]) {
    strategies.push({
  ...
  // Never allow an empty strategy set; high-risk cases still need a concise framing style.
  if (strategies.length === 0) {
    strategies.push({
      name: "educational_strategy",
      priority: 1,
      source: "fallback",
    });
  }
