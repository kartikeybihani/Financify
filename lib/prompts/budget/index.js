...
2. Use SPENDING BY CATEGORY and user profile/location as guidance for relative proportions—but always stay within the $${fmt(totalBudget)} ceiling.
3. If guidance suggests amounts that would exceed $${fmt(totalBudget)}, scale down: reduce discretionary (Entertainment, Shopping, Travel, Other, Personal Care) first, then Savings only if needed.
4. Ensure SUM of all limits = exactly $${fmt(totalBudget)}. Add up before returning.

CATEGORY RULES:
- MANDATORY: Every category from EXISTING CATEGORY NAMES above MUST be included with a non-zero limit (Loans, Travel, Education, etc.—even if only 1–2 transactions).
- Use EXACT names from EXISTING CATEGORY NAMES when present; otherwise use standard names (Food, Housing, Transportation, etc.).
- Include all transaction categories (mandatory) plus standard ones if needed; typically 8–15 total. "Savings" first, "Other" last.
- One emoji per category (use simple single-character emojis like 🍴 🏠 🚗, not compound ones like 🧖‍♀️).
- "Other" must have a non-zero limit (use it to absorb the remainder so total = $${fmt(totalBudget)}).
- Never create "INTERNAL_TRANSFER" or "Credit Card Payment" as budget categories.

Return ONLY valid JSON. Structure:
{"categories":[{"name":"Savings","icon":"💰","limit":<number>},{"name":"Food","icon":"🍴","limit":<number>},...,{"name":"Other","icon":"📝","limit":<number>}]}

CRITICAL: Sum of all "limit" values MUST equal exactly $${fmt(totalBudget)}. Put "Other" last—its limit = remainder so total equals $${fmt(totalBudget)}.

BEFORE RETURNING—verify:
✓ Savings first (limit $${fmt(effectiveSavings)}), Other last (non-zero limit)
✓ SUM of all limits = exactly $${fmt(totalBudget)}
✓ EVERY category from EXISTING CATEGORY NAMES included with non-zero limit (no omissions)
