  ...
  try {
    ...
    try {
      ...
      if (balanceData && Array.isArray(balanceData) && balanceData.length > 0) {
        ...
        // Also fetch day_change values to preserve them
        const { data: existingBalances } = await supabase
          .from("investment_balances")
          .select("currency_code, previous_total_value, total_value, id, day_change, day_change_percent")
          .eq("user_id", userId)
          .eq("snaptrade_user_id", snaptradeUserId)
          .eq("account_id", accountId)
        ...
        const cashEquivalentInHoldings = (currentHoldings || []).reduce(
          (sum, h) => sum + (isCashEq(h) ? h.market_value || 0 : 0),
          0
        );
        ...
        for (const balance of balanceData) {
          ...
          const totalValue = totalHoldingsValue + cashToAdd;

          const existingBalance = existingBalances?.find(
            (eb) => eb.currency_code === currencyCode
          );
          
          // CRITICAL: Preserve previous_total_value as the baseline for day_change calculation
          // Only use current total_value as baseline if no previous value exists (new balance)
          const previousTotalValue =
            existingBalance?.previous_total_value ??
            existingBalance?.total_value ??
            totalValue; // Only use current as fallback for NEW balances

          // CRITICAL: Preserve existing day_change values - they're computed from holdings by recalculatePortfolioMetricsFromDatabase
          // SnapTrade balance sync should NOT overwrite these with calculated values
          // The day_change will be properly computed from SUM(holdings.day_change) after this sync
          let dayChange = existingBalance?.day_change ?? null;
          let dayChangePercent = existingBalance?.day_change_percent ?? null;
          ...
          const balanceRow = {
            ...
            // CRITICAL: Preserve previous_total_value - don't overwrite with current value
            previous_total_value: previousTotalValue,
            // CRITICAL: Preserve day_change - will be recalculated from holdings
    ...
    try {
      ...
      if (positions && positions.length > 0) {
        // Fetch existing holdings WITH day_change values to preserve them
        const { data: existingHoldings } = await supabase
          .from("investment_holdings")
          .select("symbol_id, previous_market_value, market_value, symbol, day_change, day_change_percent")
          .eq("user_id", userId)
          .eq("snaptrade_user_id", snaptradeUserId)
          .eq("account_id", accountId)
          .eq("is_active", true);
        ...
        const holdingsRows = positions
          ...
          .map((holding) => {
            ...
            // CRITICAL: Preserve previous_market_value as the baseline for day_change calculation
            // Only use current market_value as baseline if no previous value exists (new holding)
            const previousMarketValue =
              existingHolding?.previous_market_value ??
              existingHolding?.market_value ??
              marketValue; // Only use current as fallback for NEW holdings

            // CRITICAL: Preserve existing day_change values - they're calculated by Finnhub
            // SnapTrade may return null/0 for day_change, but Finnhub has accurate values
            // Only calculate new day_change if we don't have existing values
            let dayChange = existingHolding?.day_change ?? null;
            let dayChangePercent = existingHolding?.day_change_percent ?? null;
            
            // If no existing day_change and we have baseline, calculate it
            if (dayChange == null && previousMarketValue != null && marketValue != null) {
            ...
            return {
              ...
              // CRITICAL: Preserve previous_market_value - don't overwrite with current value
