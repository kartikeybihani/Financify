...
const shouldExcludeFromOnboardingLLM = (tx) => {
  const effectiveCategory = String(
    tx?.new_category || tx?.top_category || tx?.sub_category || tx?.category || "",
  ).toUpperCase();
...
export const selectTopPatternsForLLM = (patternPayload, maxCount = 5) => {
  ...
  const usable = ranked.filter((pattern) => !pattern?.onboarding?.is_low_quality);
...
export const callOnboardingLLM = async ({
  ...
  const recentTxs = (Array.isArray(recentTransactions) ? recentTransactions : [])
    ...
    .map((tx) => ({
      ...
      effective_category: tx.new_category || tx.top_category || tx.category || null,
    }));
