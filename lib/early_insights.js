...
export const computePatterns = ({ transactions, months }) => {
  ...
  const txs = normalized.filter(
    (t) => !t.pending && t.kind === "expense" && t.amount > 0,
  );
  ...
  for (const tx of txs) {
    ...
    const catMonthly =
      categoryMonthlyExpense.get(category) || Array(months.length).fill(0);
    ...
    expenseByMerchant.set(
      merchant,
      (expenseByMerchant.get(merchant) || 0) + amount,
    );

    const merchMonthly =
      merchantMonthlyExpense.get(merchant) || Array(months.length).fill(0);
    ...
    const ww = categoryWeekendWeekday.get(category) || {
      weekend: 0,
      weekday: 0,
    };
    ...
    const bucket =
      tx.dayOfMonth <= 10 ? "early" : tx.dayOfMonth <= 20 ? "mid" : "late";
    ...
    const cs =
      merchantClusterMonthlySpend.get(clusterKey) ||
    ...
    categoryDowMonthlyCount.set(
      cdKey,
      (categoryDowMonthlyCount.get(cdKey) || 0) + 1,
    );
  }
  ...
  const habitMerchants = [...merchantStats.values()]
    ...
    .map((s) => {
      ...
      const conf = clamp01(
        0.35 + monthsPresent / 10 + Math.min(0.35, avgPerMonth / 8),
      );
      return {
        ...s,
        monthsPresent,
  ...
  const routines = [...dowStats.values()]
    ...
    .map((s) => {
      ...
      const avgPerMonth = s.totalCount / months.length;
      const conf = clamp01(
        0.3 + monthsPresent / 10 + Math.min(0.4, avgPerMonth / 10),
      );
  ...
  for (const d of categoryDrift.slice(0, 3)) {
    ...
    const meaningful =
      Math.abs(d.delta) >= 40 || (d.pct !== null && Math.abs(d.pct) >= 0.35);
  ...
  if (Math.abs(shareDelta) >= 0.12 && recentTotal > 0 && priorTotal > 0) {
    patterns.push({
      ...
      title:
        shareDelta > 0 ? "Weekends got more expensive" : "Weekends got calmer",
  ...
  for (const c of clusterPatterns) {
    patterns.push({
      ...
      confidence: clamp01(
        0.5 +
          Math.min(0.4, c.memberCount / 6) +
          Math.min(0.1, c.total / (totalExpense || 1)),
      ),
  ...
  const contextualRoutines = [...contextualStats.values()]
    ...
    .map((s) => {
      ...
      const avgPerMonth = s.totalCount / months.length;
      const conf = clamp01(
        0.45 + monthsPresent / 12 + Math.min(0.35, avgPerMonth / 8),
      );
      return { ...s, monthsPresent, avgPerMonth, confidence: conf };
    })
  ...
  const spikeMonths = months.filter(
    (m, i) => monthlyMean > 0 && monthlyTotals[i] > monthlyMean + monthlySd,
  );
  ...
  for (const t of txs) {
    ...
    const bucket = recentMonths.has(t.month)
      ? "recent"
      : earlyMonths.has(t.month)
        ? "prior"
        : null;
    if (!bucket) continue;
  ...
  if (
    Math.abs(lateDelta) >= 0.12 &&
    lateCounts.priorTotal >= 30 &&
    lateCounts.recentTotal >= 30
  ) {
    patterns.push({
      ...
      title:
        lateDelta > 0
          ? "You’re pushing more spending to the end of the month"
          : "End-of-month spending eased up",
...
const ONBOARDING_PATTERN_TYPE_PRIORITY = {
  spend_concentration: 120,
  behavior_category_drift: 110,
  behavior_weekend_shift: 100,
  behavior_monthly_volatility: 95,
  behavior_late_month_shift: 92,
  routine_day_of_week: 88,
  identity_weekend_category: 84,
  identity_end_of_month_cluster: 80,
  behavior_merchant_spike: 68,
  identity_contextual_routine: 62,
  merchant_stickiness: 52,
  identity_merchant_cluster: 44,
};

const ONBOARDING_THEME_PRIORITY = {
  fixed_cost_anchor: 140,
  spending_drift: 125,
  timing_pressure: 115,
  recurring_structure: 92,
  merchant_behavior: 28,
  low_signal: 10,
};
...
const getTopCategoryShare = (pattern) => {
  const topCategory = pattern?.evidence?.topCategories?.[0];
  return typeof topCategory?.share === "number" ? topCategory.share : 0;
};

const getTopCategoryName = (pattern) =>
  String(pattern?.evidence?.topCategories?.[0]?.category || "").trim();

const getPatternTheme = (pattern) => {
  const type = String(pattern?.type || "");
  if (type === "spend_concentration") {
    const topCategory = getTopCategoryName(pattern).toUpperCase();
    return FIXED_COST_CATEGORIES.has(topCategory)
      ? "fixed_cost_anchor"
      : "spending_drift";
  }
  ...
  if (
    ...
    return "recurring_structure";
  }

  if (type === "merchant_stickiness" || type === "identity_merchant_cluster") {
...
const getWhyThisMatters = (pattern, theme) => {
  if (theme === "fixed_cost_anchor") {
    return "This looks like the fixed anchor shaping the rest of the month.";
  }
  if (theme === "spending_drift") {
    return "This shows whether spending is getting heavier or lighter over time.";
  }
  if (theme === "timing_pressure") {
    return "This reveals when spending tends to bunch up and create pressure.";
  }
  if (theme === "recurring_structure") {
    return "This is stable enough for Finny to separate normal rhythm from change.";
  }
  if (theme === "merchant_behavior") {
    return "Only use this if stronger system-level patterns are absent.";
  }
  return "Use only if it clearly explains the user's money rhythm.";
};

const getWhatFinnyWillWatchNext = (pattern, theme) => {
  if (theme === "fixed_cost_anchor") {
    return "whether flexible expenses stay calm around this fixed load or start drifting";
  }
  if (theme === "spending_drift") {
    return "whether this category keeps moving in the same direction or starts settling back";
  }
  if (theme === "timing_pressure") {
    return "whether spending keeps bunching up in the same part of the week or month";
  }
  if (theme === "recurring_structure") {
    return "whether this rhythm stays normal or starts breaking pattern";
  }
  if (theme === "merchant_behavior") {
    return "whether this habit stays minor or starts becoming a bigger pattern";
  }
  return "what changes first if this pattern shifts";
};

const isHighValueMerchantPattern = (pattern) => {
  const type = String(pattern?.type || "");
  if (type === "merchant_stickiness") {
    const totalCount = Number(pattern?.evidence?.totalCount || 0);
    const medianTxnAmount = Number(pattern?.evidence?.medianTxnAmount || 0);
    return totalCount >= 18 && medianTxnAmount >= 25;
  }

  if (type === "behavior_merchant_spike") {
    const spikeSpend = Number(pattern?.evidence?.spikeSpend || 0);
    const ratio = Number(pattern?.evidence?.spikeVsAverage || 0);
    return spikeSpend >= 250 && ratio >= 2.2;
  }

  if (type === "identity_merchant_cluster") {
    const avgMonthlySpend = Number(pattern?.evidence?.avgMonthlySpend || 0);
    const memberCount = Number(pattern?.evidence?.memberNames?.length || 0);
    return avgMonthlySpend >= 250 && memberCount >= 2;
  }

  return true;
};

const getPatternWowScore = (pattern, theme) => {
  const type = String(pattern?.type || "");
  const confidence = Number(pattern?.confidence || 0);
  let score = (ONBOARDING_THEME_PRIORITY[theme] || 0) + confidence * 15;

  if (type === "spend_concentration") {
    const share = getTopCategoryShare(pattern);
    score += clampNumber(Math.round(share * 100), 0, 35);
    if (theme === "fixed_cost_anchor") score += 18;
  }

  if (type === "behavior_category_drift") {
    const delta = Math.abs(Number(pattern?.evidence?.delta || 0));
    const pct = Math.abs(Number(pattern?.evidence?.pctChange || 0));
    score += clampNumber(delta / 6, 0, 24) + clampNumber(pct * 35, 0, 18);
  }
  ...
  if (
    ...
    const deltaPts = Math.abs(Number(pattern?.evidence?.delta || 0)) * 100;
    score += clampNumber(deltaPts, 0, 26);
  }

  if (type === "identity_end_of_month_cluster") {
    const share = Number(pattern?.evidence?.endOfMonthShare || 0) * 100;
    score += clampNumber(share / 2, 0, 20);
  }

  if (type === "behavior_monthly_volatility") {
    const cv = Number(pattern?.evidence?.coefficientOfVariation || 0);
    score += clampNumber(cv * 70, 0, 22);
  }

  if (theme === "merchant_behavior" && !isHighValueMerchantPattern(pattern)) {
    score -= 90;
  }
...
const annotatePatternForOnboarding = (pattern) => {
  const theme = getPatternTheme(pattern);
  const wowScore = getPatternWowScore(pattern, theme);
  const qualityFloor = theme === "merchant_behavior" ? 80 : 95;
  const isLowQuality =
    wowScore < qualityFloor ||
    (theme === "merchant_behavior" && !isHighValueMerchantPattern(pattern));
  ...
  return {
    ...
    onboarding: {
      theme,
      wow_score: wowScore,
      why_this_matters: getWhyThisMatters(pattern, theme),
      watch_next: getWhatFinnyWillWatchNext(pattern, theme),
...
const looksLikePersonalCounterparty = (tx) => {
  const raw = String(tx?.merchant_name || tx?.name || "").trim();
  if (!raw) return false;

  const upper = raw.toUpperCase();
  if (SENSITIVE_PAYMENT_KEYWORDS.some((keyword) => upper.includes(keyword))) {
    return true;
  }

  const stripped = raw
    .replace(/^(payment to|payment from|sent to|received from)\s+/i, "")
    .replace(/[0-9#*._-]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  ...
  if (!isLikelyPersonName) return false;

  return !BUSINESS_NAME_HINTS.some((hint) => upper.includes(hint));
};

const shouldExcludeFromOnboardingLLM = (tx) => {
  ...
  if (
    effectiveCategory.includes("INTERNAL_TRANSFER") ||
    effectiveCategory.includes("TRANSFER")
  ) {
    return true;
  }

  return looksLikePersonalCounterparty(tx);
};

export const selectTopPatternsForLLM = (patternPayload, maxCount = 5) => {
  ...
  const ranked = [...patterns]
    .map(annotatePatternForOnboarding)
    .sort((a, b) => {
      const wowDiff =
        Number(b?.onboarding?.wow_score || 0) -
        Number(a?.onboarding?.wow_score || 0);
      if (wowDiff !== 0) return wowDiff;

      const themeDiff =
        (ONBOARDING_THEME_PRIORITY[b?.onboarding?.theme] || 0) -
        (ONBOARDING_THEME_PRIORITY[a?.onboarding?.theme] || 0);
      if (themeDiff !== 0) return themeDiff;

      const typeDiff =
        (ONBOARDING_PATTERN_TYPE_PRIORITY[b?.type] || 0) -
        (ONBOARDING_PATTERN_TYPE_PRIORITY[a?.type] || 0);
      if (typeDiff !== 0) return typeDiff;

      const scoreDiff =
        getPatternFrequencyScore(b) - getPatternFrequencyScore(a);
      if (scoreDiff !== 0) return scoreDiff;

      const monthsDiff =
        (typeof b?.evidence?.monthsPresent === "number"
          ? b.evidence.monthsPresent
          : 0) -
        (typeof a?.evidence?.monthsPresent === "number"
          ? a.evidence.monthsPresent
          : 0);
  ...
  const usable = ranked.filter(
    (pattern) => !pattern?.onboarding?.is_low_quality,
  );
  ...
  for (const theme of priorityThemes) {
    const match = pool.find(
      (pattern) =>
        pattern?.onboarding?.theme === theme && !seenKeys.has(pattern?.key),
    );
    if (!match) continue;
    selected.push(match);
  ...
  return selected.slice(0, Math.max(1, maxCount));
};
...
export const callOnboardingLLM = async ({
  ...
  const resolvedModel =
    String(model || "meta-llama/llama-4-scout").trim() ||
  ...
  // Slim down recent transactions to only the fields the LLM needs for
  // category-verification. Keep the payload small to stay within token limits.
  const recentTxs = (
    Array.isArray(recentTransactions) ? recentTransactions : []
  )
    .filter((tx) => !shouldExcludeFromOnboardingLLM(tx))
    .slice(0, 160) // keep enough texture for trend detection without bloating tokens
    .map((tx) => ({
      name: tx.name || tx.merchant_name || null,
      merchant_name: tx.merchant_name || null,
      amount: tx.amount ?? null,
      date: tx.date || null,
      category: tx.category || null,
      top_category: tx.top_category || null,
      sub_category: tx.sub_category || null,
      effective_category:
        tx.new_category || tx.top_category || tx.category || null,
    }));
  ...
  // console.log("[ONBOARDING_LLM] system_prompt");
  // console.log(system);
  // console.log("[ONBOARDING_LLM] user_prompt");
  // console.log(user);
  ...
  try {
    ...
    try {
      ...
      if (err.name === "AbortError") {
        err.isTimeout = true;
      } else if (typeof err.isNetworkError !== "boolean") {
    ...
    if (!content) {
      const err = new Error("No LLM content returned");
      err.stage = "llm_request";
      err.provider = "openrouter";
      err.httpStatus = res.status;
      err.model = resolvedModel;
    ...
    return {
      ok: false,
      raw: content,
      rawStripped: String(content || "").trim(),
      model: resolvedModel,
      httpStatus: res.status,
      latencyMs: Date.now() - startedAt,
      requestPayload,
...
export const callAccountCompletenessLLM = async ({
  openRouterApiKey,
  fetchFn,
  transactions,
  model = "meta-llama/llama-4-scout",
  ...
  const transactionsForAnalysis = (
    ...
    name: tx.name,
    category: tx.new_category || tx.top_category || tx.category || "Other",
  }));

  const { system, user } = buildAccountCompletenessPrompt(
...
export const buildEarlyInsightsJson = async ({
  ...
  // Extract last 60 days of transactions for LLM category verification and
  // slightly richer short-term behavior context.
  const sixtyDaysAgo = new Date();
  sixtyDaysAgo.setDate(sixtyDaysAgo.getDate() - 60);
  const sixtyDayCutoff = sixtyDaysAgo.toISOString().slice(0, 10);
  const recentTransactions = filtered.filter((tx) => tx.date >= sixtyDayCutoff);
