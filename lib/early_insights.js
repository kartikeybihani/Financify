...
export const callOnboardingLLM = async ({
  ...
  const recentTxs = (Array.isArray(recentTransactions) ? recentTransactions : [])
    .slice(0, 100) // cap to avoid blowing up tokens
    .map((tx) => ({
      ...
      top_category: tx.top_category || tx.new_category || null,
    }));
...
export const buildEarlyInsightsJson = async ({
  ...
  // Extract last 30 days of transactions for LLM category verification.
