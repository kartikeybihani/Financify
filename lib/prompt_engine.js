  ...
  if (financialData.categoryDetails) {
    ...
    if (
      ...
      if (
        ...
        if (catDetails.total_sent !== undefined) {
          breakdownLines.push(
            `Sent: $${catDetails.total_sent.toLocaleString(undefined, {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            })} (${catDetails.sent_count || 0} transaction${(catDetails.sent_count || 0) !== 1 ? "s" : ""})`,
          );
        }
        ...
        if (catDetails.total_received !== undefined) {
          breakdownLines.push(
            `Received: $${catDetails.total_received.toLocaleString(undefined, {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            })} (${catDetails.received_count || 0} transaction${(catDetails.received_count || 0) !== 1 ? "s" : ""})`,
          );
        }
        ...
        sectionContent += `\nTotal: ${catDetails.txn_count || catDetails.transactions.length} transaction${catDetails.txn_count !== 1 && catDetails.transactions.length !== 1 ? "s" : ""}`;
      } else if (
        ...
        sectionContent += `\n\nTotal: $${Math.abs(
          ...
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })} across ${catDetails.txn_count || catDetails.transactions.length} transaction${catDetails.txn_count !== 1 && catDetails.transactions.length !== 1 ? "s" : ""}`;
      } else {
        sectionContent += `\n\nTotal: ${catDetails.transactions.length} transaction${catDetails.transactions.length !== 1 ? "s" : ""}`;
      }
...
- COMPLEX QUESTIONS (how/why explanations, analysis, comparisons, multi-step guidance): 7 sentences max
- PHILOSOPHICAL/REFLECTIVE QUESTIONS (e.g., "What would change if you understood your money better?", "What would you do if money wasn't a constraint?"): Keep concise (2-4 sentences max). These are meant to spark reflection, not provide lengthy explanations.
...
function buildBudgetGenerationPrompt({
  ...
  const spendingAnalysis = Object.keys(categorySpending)
    ...
    .map((cat) => {
      ...
      return `- ${cat}: $${total3Months.toFixed(2)} total over 3 months ($${monthlyAvg.toFixed(2)}/month avg, ${txCount} transactions)`;
    })
    .join("\n");
  ...
  const defaultSavingsAmount = savingsAmount || Math.round(income * 0.15); // Default to 15% if not specified
...
USER PROFILE:
${userContext.length > 0 ? userContext.join("\n") : "No profile information available"}${locationGuidance}

FINANCIAL CONTEXT:
- Monthly Income: $${income.toLocaleString()}
${savingsAmount ? `- Monthly Savings Goal: $${savingsAmount.toLocaleString()}` : `- Monthly Savings Goal: $${defaultSavingsAmount.toLocaleString()} (15% default)`}
...
- CRITICAL: ALWAYS include "Savings" as the FIRST category with limit = $${(savingsAmount || defaultSavingsAmount).toLocaleString()}
...
- Calculate: Sum all category limits = $${totalBudget.toLocaleString()}
- If the sum doesn't match, adjust the "Other" category limit to make up the difference
- Example: If sum is $${(totalBudget - 100).toLocaleString()} and target is $${totalBudget.toLocaleString()}, set "Other" to $100 more
...
- Count total categories: Should be 8-15
- Verify "Savings" is included as the FIRST category with limit = $${(savingsAmount || defaultSavingsAmount).toLocaleString()}
- Verify each category name matches existing category names exactly (same capitalization, spacing, punctuation) - EXCEPT "Savings" which is always included
- Verify each category name is 13 characters or less
- Verify "Other" is included as the last category
- VERIFY TOTAL: Sum all limits = $${totalBudget.toLocaleString()} (MUST BE EXACT - equals full income)

TASK:
Create a personalized monthly budget that:
1. CRITICAL: ALWAYS includes "Savings" as the FIRST category with limit = $${(savingsAmount || defaultSavingsAmount).toLocaleString()}
2. Uses ACTUAL transaction spending patterns as the foundation (see SPENDING ANALYSIS above)
3. For recurring large expenses (like housing), use the ACTUAL transaction amount as the monthly budget
4. For categories with multiple transactions, use monthly average as baseline, then adjust slightly for improvement
5. Accounts for user's lifestyle, occupation, age, and LOCATION (cost of living differences)${age ? ` (age ${age})` : ""}${occupation ? ` (${occupation})` : ""}${location ? ` (${location})` : ""}
6. Includes essential categories relevant to their situation
7. Uses appropriate emojis for each category
8. Ensures all category names are 13 characters or less
9. CRITICAL: Use existing category names from transactions above - DO NOT create completely new category names. Match the exact naming convention used in existing categories to ensure proper transaction matching later. EXCEPTION: "Savings" is always included even if not in transactions.
...
- CRITICAL: "Savings" MUST be the FIRST category with limit = $${(savingsAmount || defaultSavingsAmount).toLocaleString()}
...
- ALWAYS include essential categories: Savings (FIRST), Housing, Food, Transportation, Entertainment, Personal Care, Shopping, Travel, Other (but use existing category names if they exist)
- "Other" must always be included as the last category with an appropriate limit (typically 5-15% of remaining budget after savings)
...
NOTE: "Savings" MUST be the FIRST category in the array with limit = $${(savingsAmount || defaultSavingsAmount).toLocaleString()}

FINAL REQUIREMENTS:
- Return 8-15 categories (including "Savings" and "Other")
- CRITICAL: "Savings" MUST be the FIRST category with limit = $${(savingsAmount || defaultSavingsAmount).toLocaleString()}
- Use emojis for icons (single emoji per category)
- CRITICAL BUDGET CONSTRAINT: The SUM of ALL category limits (including "Savings" and "Other") MUST equal EXACTLY $${totalBudget.toLocaleString()} (the full monthly income). This is ABSOLUTE and NON-NEGOTIABLE. Calculate the sum before returning and adjust "Other" if needed to reach exactly $${totalBudget.toLocaleString()}.
- Category names should be clear and concise, and MUST NOT exceed 13 characters in length
- CRITICAL CATEGORY NAMING: Use the EXACT category names from the "EXISTING CATEGORY NAMES FROM TRANSACTIONS" section above. Match the naming convention exactly (capitalization, spacing, punctuation) to ensure proper transaction matching. EXCEPTION: "Savings" is always included even if not in transactions.
- Include categories that match their spending patterns but also suggest improvements
- CRITICAL: "Other" must always be included as the last category in the array with an appropriate limit amount. Do NOT set it to 0 - allocate a realistic amount for miscellaneous/unexpected expenses.
- Account for location cost of living: adjust housing, transportation, entertainment, and dining budgets based on whether the location is a major city, small city, college town, etc.

VALIDATION CHECKLIST BEFORE RETURNING:
✓ "Savings" is the FIRST category with limit = $${(savingsAmount || defaultSavingsAmount).toLocaleString()}
✓ All category names match existing category names exactly (except "Savings")
✓ All category names are 13 characters or less
✓ "Other" is included as the last category
✓ Total of all category limits = $${totalBudget.toLocaleString()} (EXACTLY - equals full income, verify this!)
✓ 8-15 categories total
...
${transactionLines.length > 0 ? transactionLines.join("\n") : "No transaction categories found"}
