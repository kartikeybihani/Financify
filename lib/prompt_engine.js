...
function prioritizeMemories(memories, query, intent) {
  console.log(
    `🧠 [PROMPT_ENGINE] prioritizeMemories called:`,
    JSON.stringify(
      {
        memoriesProvided: !!memories,
        memoriesIsArray: Array.isArray(memories),
        memoriesLength: memories?.length || 0,
        query: query?.substring(0, 50) || null,
        intent: intent || null,
        sampleMemory: memories?.[0]
          ? {
              id: memories[0].id,
              hasContent: !!memories[0].content,
              hasSummary: !!memories[0].summary,
              contentPreview: memories[0].content?.substring(0, 100) || null,
  ...
  if (!memories || !Array.isArray(memories) || memories.length === 0) {
    console.log(
      `🧠 [PROMPT_ENGINE] prioritizeMemories: No memories to prioritize (empty or invalid)`
    );
    return [];
  }
  ...
  const synthesizedMemories = topMemories.map((memory) => {
  ...
  console.log(
    `🧠 [PROMPT_ENGINE] prioritizeMemories result:`,
    JSON.stringify(
      {
        inputCount: memories.length,
        topMemoriesCount: topMemories.length,
        synthesizedCount: synthesizedMemories.length,
        sampleSynthesized: synthesizedMemories[0]
          ? {
              insightPreview:
                synthesizedMemories[0].insight?.substring(0, 100) || null,
              type: synthesizedMemories[0].type,
              relevance: synthesizedMemories[0].relevance,
            }
          : null,
      },
      null,
...
- Student loans: MUST mention optimization opportunities`;
  ...
  personalization: (
    ...
    console.log(
      `🧠 [PROMPT_ENGINE] personalization module called:`,
      JSON.stringify(
        {
          memoriesProvided: !!memories,
          memoriesIsArray: Array.isArray(memories),
          ...
          hasIntentContext: !!intentContext,
          sampleMemory: memories?.[0]
            ? {
                hasInsight: !!memories[0].insight,
                insightPreview: memories[0].insight?.substring(0, 100) || null,
...
- Provide complete answers that fully address the question, regardless of complexity`;
    ...
    if (memories && memories.length > 0) {
      console.log(
        `🧠 [PROMPT_ENGINE] Adding ${memories.length} memories to personalization section`
      );
...
before mentioning financial numbers.`;
      ...
      console.log(
        `🧠 [PROMPT_ENGINE] No memories to add to personalization section (memories: ${memories}, length: ${
          memories?.length || 0
        })`
      );
    }
...
- Never mention data sources: Don't say "based on your data" or "data that I have" or "after checking your data"
...
function buildContextAwarePrompt(
  ...
  console.log(
    `🧠 [PROMPT_ENGINE] Extracting memories from context:`,
    JSON.stringify(
      {
        ...
        rawMemoriesLength: context.memory?.memories?.length || 0,
        totalCount: context.memory?.totalCount || 0,
        memoryKeys: context.memory ? Object.keys(context.memory) : [],
        sampleRawMemory: context.memory?.memories?.[0]
          ? {
              id: context.memory.memories[0].id,
              hasContent: !!context.memory.memories[0].content,
              hasSummary: !!context.memory.memories[0].summary,
              contentPreview:
                context.memory.memories[0].content?.substring(0, 100) || null,
              similarity: context.memory.memories[0].similarity,
            }
          : null,
      },
      null,
  ...
  console.log(
    `🧠 [PROMPT_ENGINE] After prioritizeMemories:`,
    JSON.stringify(
      {
        prioritizedMemoriesLength: memories.length,
        samplePrioritizedMemory: memories[0]
          ? {
              hasInsight: !!memories[0].insight,
              insightPreview: memories[0].insight?.substring(0, 100) || null,
              type: memories[0].type,
