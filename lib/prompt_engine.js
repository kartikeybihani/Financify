...
function buildBudgetGenerationPrompt({
  ...
  const transactionSummary = transactions
    ...
    .map((tx) => {
      const category = tx.new_category || tx.top_category || tx.category || "Other";
...
RECENT SPENDING PATTERNS (last 6 months):
...
2. Accounts for user's lifestyle${age ? ` (age ${age})` : ""}${occupation ? ` (${occupation})` : ""}
...
- Always include: Housing, Food/Groceries, Transportation, Health, Savings
