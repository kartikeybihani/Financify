...
2. recent_transactions — the user's last 30 days of raw transactions (merchant name, amount, date, and the category label assigned by the bank/Plaid)
...
Look at the merchant names and amounts in recent_transactions. If the top pattern's category (e.g. "Other") is inflated because it contains transactions that clearly belong to a different real category (e.g. large recurring payments to a landlord, property manager, or utility company), you MUST:
  ...
  - If after mentally re-categorizing, the pattern is no longer meaningful (e.g. "Other" drops to a small amount), write about patterns[1] instead.
  - If BOTH computed patterns are invalidated, fall back to the recent_transactions directly: scan the last 30 days, find the most interesting repeating merchant or spending habit, and write about that instead.
Do NOT mention this verification step or Plaid/bank labels to the user.

CRITICAL: You must ALWAYS produce a meaningful insight. Never return null values. There is always something worth surfacing — a frequently visited merchant, a noticeable spending rhythm, a go-to spot. Use the recent_transactions as your safety net.

Input JSON includes:
- user_profile.first_name
- analysis_window
- patterns (array, up to 2) — already sorted highest-frequency first
- recent_transactions (array) — last 30 days, each with { name, merchant_name, amount, date, category, top_category }

Priority order for choosing what to write about:
1. patterns[0] (if it survives category verification)
2. patterns[1] (if patterns[0] was invalidated)
3. The most prominent repeating merchant or habit visible in recent_transactions (if both patterns were invalidated)
 ...
 * @returns {string} - Formatted prompt for LLM
 */
function buildRecurringAnalysisPrompt(transactions, plaidStreams = []) {
