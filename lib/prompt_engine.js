...
- CRITICAL: The SUM of ALL category limits MUST equal EXACTLY $${availableBudget.toLocaleString()}
- Calculate: Sum all category limits = $${availableBudget.toLocaleString()}
- If the sum doesn't match, adjust the "Other" category limit to make up the difference
- Example: If sum is $${(availableBudget - 100).toLocaleString()} and target is $${availableBudget.toLocaleString()}, set "Other" to $100 more
- DOUBLE-CHECK: Add up all limits before returning - the total MUST be exactly $${availableBudget.toLocaleString()}

STEP 4: VALIDATE BEFORE RETURNING
- Count total categories: Should be 8-15
- Verify each category name matches existing category names exactly (same capitalization, spacing, punctuation)
- Verify each category name is 13 characters or less
- Verify "Other" is included as the last category
- VERIFY TOTAL: Sum all limits = $${availableBudget.toLocaleString()} (MUST BE EXACT)

TASK:
Create a personalized monthly budget that:
1. Uses ACTUAL transaction spending patterns as the foundation (see SPENDING ANALYSIS above)
2. For recurring large expenses (like housing), use the ACTUAL transaction amount as the monthly budget
3. For categories with multiple transactions, use monthly average as baseline, then adjust slightly for improvement
4. Accounts for user's lifestyle, occupation, age, and LOCATION (cost of living differences)${age ? ` (age ${age})` : ""}${occupation ? ` (${occupation})` : ""}${location ? ` (${location})` : ""}
5. Includes essential categories relevant to their situation
6. ${savingsAmount ? `Ensures savings goal of $${savingsAmount.toLocaleString()} is achievable` : "Includes a reasonable savings allocation (15-20% of income)"}
7. Uses appropriate emojis for each category
8. Ensures all category names are 13 characters or less
9. CRITICAL: Use existing category names from transactions above - DO NOT create completely new category names. Match the exact naming convention used in existing categories to ensure proper transaction matching later.
...
- ALWAYS include essential categories: Housing, Food, Transportation, Entertainment, Personal Care, Shopping, Travel, Other (but use existing category names if they exist)
- "Other" must always be included as the last category with an appropriate limit (typically 5-15% of available budget)
...
- CRITICAL BUDGET CONSTRAINT: The SUM of ALL category limits (including "Other") MUST equal EXACTLY $${availableBudget.toLocaleString()}. This is ABSOLUTE and NON-NEGOTIABLE. Calculate the sum before returning and adjust "Other" if needed to reach exactly $${availableBudget.toLocaleString()}.
- Category names should be clear and concise, and MUST NOT exceed 13 characters in length
- CRITICAL CATEGORY NAMING: Use the EXACT category names from the "EXISTING CATEGORY NAMES FROM TRANSACTIONS" section above. Match the naming convention exactly (capitalization, spacing, punctuation) to ensure proper transaction matching.
...
✓ All category names match existing category names exactly
✓ All category names are 13 characters or less
✓ "Other" is included as the last category
✓ Total of all category limits = $${availableBudget.toLocaleString()} (EXACTLY - verify this!)
