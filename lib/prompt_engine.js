...
function buildContextAwarePrompt(
  ...
  selectedModules.push(
    `personality: ${personalityKey} (from emotionalState: ${userState.emotionalState})`
  );
  ...
  if (selectedStrategies.length > 0) {
    selectedModules.push(
      `response_strategy: [${selectedStrategies.join(
        ", "
      )}] (from needs: [${userState.needs.join(", ")}])`
    );
  }
  ...
  // Consolidated module selection log
  console.log(`\n📦 [PROMPT_MODULES] Selected:`);
