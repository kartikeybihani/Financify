...
${transactionLines.join("\n")}`);
  ...
  if (Array.isArray(spendByCategoryCurrentMonth) && spendByCategoryCurrentMonth.length > 0) {
...
${categoryLines.join("\n")}`);
  ...
  if (Array.isArray(spendByCategoryLastMonth) && spendByCategoryLastMonth.length > 0) {
...
${categoryLines.join("\n")}`);
  ...
  if (
    (!Array.isArray(spendByCategoryCurrentMonth) || spendByCategoryCurrentMonth.length === 0) &&
    (!Array.isArray(spendByCategoryLastMonth) || spendByCategoryLastMonth.length === 0) &&
    Array.isArray(spendByCategory) && spendByCategory.length > 0
  ) {
  ...
  if (financialData.categoryDetails) {
    ...
    if (
      ...
      const txnLines = catDetails.transactions.map((txn) => {
        ...
        const direction = txn.direction || (amount > 0 ? "sent" : amount < 0 ? "received" : "neutral");
        ...
        const isBidirectional = catDetails.merchant && 
          ((catDetails.total_sent !== undefined && catDetails.total_received !== undefined) ||
           (catDetails.sent_count !== undefined && catDetails.received_count !== undefined) ||
      ...
      if (isBidirectional && (catDetails.total_sent !== undefined || catDetails.total_received !== undefined)) {
        ...
        if (catDetails.total_sent !== undefined) {
          breakdownLines.push(`Sent: $${catDetails.total_sent.toLocaleString(undefined, {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
        ...
        if (catDetails.total_received !== undefined) {
          breakdownLines.push(`Received: $${catDetails.total_received.toLocaleString(undefined, {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
      ...
      } else if (catDetails.total_spend !== undefined && catDetails.total_spend !== null) {
        // Unidirectional merchant or category - show total spend
        sectionContent += `\n\nTotal: $${Math.abs(catDetails.total_spend).toLocaleString(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
...
you must pause and ask clarifying questions before giving advice.`;
    ...
    if (emptyMerchantQueries && Array.isArray(emptyMerchantQueries) && emptyMerchantQueries.length > 0) {
      const merchantList = emptyMerchantQueries.map(q => 
        `- ${q.merchant} (${q.periodDisplay})`
      ).join("\n");
