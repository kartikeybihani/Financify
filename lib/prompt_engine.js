...
function detectUserState(message, financialData = {}) {
  ...
  // Filter out false positives: "emergency fund" is informational, not panic-inducing
  const panicMatches = panicSignals.filter((signal) => lower.includes(signal));
  
  // Also check for standalone "emergency" (excluding "emergency fund" context)
  if (lower.includes("emergency") && !lower.includes("emergency fund")) {
    // Only count as panic if it's clearly about an urgent situation
    const emergencyContext = lower.match(/emergency[^f]*?/);
    if (emergencyContext && (
      lower.includes("need") || 
      lower.includes("help") || 
      lower.includes("now") ||
...
- Budgets: monthly only (set/edit by category)
- Transactions: view history (filter options)
- Goals: create/edit on Goals tab
- Accounts: view/manage/add on Home tab
- Insights: analytics tab

CRITICAL: NEVER suggest features that don't exist. Examples of NON-EXISTENT features:
- Any tabs, filters, or buttons not listed above

Never direct users to non-existent UI elements like "Credit Card tab", "Filter by score", or "Apply" buttons in Finny. Never mention competitors (Mint, YNAB, Personal Capital) or features we don't have.`,
...
- NEVER reference non-existent app features. Only mention: Budgets, Transactions, Goals, Accounts, Insights tabs. If suggesting credit card advice, provide general recommendations only - never direct to UI elements that don't exist.`;
...
function buildContextAwarePrompt(
  ...
  selectedModules.push(`communication_style: ${style}`);
  ...
  selectedModules.push(`personality: ${personalityKey} (from emotionalState: ${userState.emotionalState})`);
  ...
  userState.needs.forEach((need) => {
    ...
    if (strategy) {
      ...
      selectedStrategies.push(need);
    }
  });
  if (selectedStrategies.length > 0) {
    selectedModules.push(`response_strategy: [${selectedStrategies.join(", ")}] (from needs: [${userState.needs.join(", ")}])`);
  }

  // 6. Empathetic engagement (always included)
  promptParts.push(PROMPT_MODULES.empathetic());
  selectedModules.push("empathetic (always)");

  // 7. Response guidelines (modified based on state)
  promptParts.push(PROMPT_MODULES.response_guidelines(userState));
  selectedModules.push(`response_guidelines (modified by state)`);
  
  // Log which modules were selected based on userState
  console.log(`🎯 [PROMPT_ENGINE] Selected prompt modules based on userState:`);
