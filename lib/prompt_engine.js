...
function buildBudgetGenerationPrompt({
  ...
  // Format transactions for context (up to 200 transactions from last 3 months)
  const transactionSummary = transactions
    .slice(0, 200) // Last 200 transactions (from 3 months of data)
    .map((tx) => {
      const category =
  ...
  const availableBudget = Math.round(income - (savingsAmount || income * 0.2));
...
${savingsAmount ? `- Monthly Savings Goal: $${savingsAmount.toLocaleString()}` : "- Monthly Savings Goal: Not specified"}
- Available Budget (Income - Savings): $${availableBudget.toLocaleString()}

RECENT TRANSACTIONS (last 3 months, up to 200 transactions):
...
1. Is realistic but optimistic (helps user improve spending habits)
2. Accounts for user's lifestyle, occupation, age, and LOCATION (cost of living differences)${age ? ` (age ${age})` : ""}${occupation ? ` (${occupation})` : ""}${location ? ` (${location})` : ""}
3. Includes essential categories relevant to their situation
4. Allocates appropriate amounts based on spending patterns but with room for improvement
5. ${savingsAmount ? `Ensures savings goal of $${savingsAmount.toLocaleString()} is achievable` : "Includes a reasonable savings allocation (15-20% of income)"}
6. Uses appropriate emojis for each category
7. Ensures all category names are 13 characters or less
8. CRITICAL: Use existing category names from transactions above - DO NOT create completely new category names. Match the exact naming convention used in existing categories to ensure proper transaction matching later. If you need to add categories that don't exist, use similar naming patterns and conventions as the existing ones.

Consider:
- CRITICAL CATEGORY NAMING: Use the EXACT category names from "EXISTING CATEGORY NAMES FROM TRANSACTIONS" above. Do not create new category names - reuse existing ones to ensure proper transaction matching. Follow the same naming convention, capitalization, and formatting as shown in the existing categories list.
- If existing categories are available: prioritize using those exact names. Only add new categories if absolutely necessary, and when doing so, follow the same naming patterns (e.g., if existing categories use title case like "Food & Drink", use similar formatting).
...
- ALWAYS include essential and obvious categories by default: Housing, Food, Groceries, Travel, Shopping, Entertainment, Personal Care, Transportation, Health, Subscriptions (but use existing category names if they exist)
- Don't skip obvious categories - users need these for accurate tracking
- Always include "Other" as the final category with an appropriate limit based on miscellaneous spending patterns (typically 5-15% of available budget for unexpected expenses, small purchases, and uncategorized items)
- If no transaction history: create a standard budget based on income, user profile, and location
- Include categories based on their actual spending patterns (if available)
- For new users with no history: suggest common categories with reasonable defaults based on location and income
...
Requirements:
- Return 8-15 categories
- Use emojis for icons (single emoji per category)
- Limit amounts should be realistic monthly budgets based on location and user profile
- CRITICAL BUDGET CONSTRAINT: The SUM of ALL category limits (including "Other") MUST equal exactly $${availableBudget.toLocaleString()}. This is ABSOLUTE - the total must be exactly $${availableBudget.toLocaleString()}. Double-check your math before returning.
- Category names should be clear and concise, and MUST NOT exceed 13 characters in length
- CRITICAL CATEGORY NAMING: Use the EXACT category names from the "EXISTING CATEGORY NAMES FROM TRANSACTIONS" section above. Match the naming convention exactly (capitalization, spacing, punctuation) to ensure proper transaction matching. Only create new category names if absolutely necessary, and follow the same naming patterns as existing categories.
- Include categories that match their spending patterns but also suggest improvements
- CRITICAL: "Other" must always be included as the last category in the array with an appropriate limit amount (typically 5-15% of available budget) based on miscellaneous spending patterns and unexpected expenses. Do NOT set it to 0 - allocate a realistic amount for miscellaneous/unexpected expenses.
