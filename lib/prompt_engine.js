...
${goalLines.join("\n")}`);
  ...
  if (financialData.categoryDetails) {
    ...
    console.log("📊 [PROMPT_ENGINE] categoryDetails pack received:", {
      ...
      transactionCount: Array.isArray(catDetails.transactions) ? catDetails.transactions.length : 0,
      sampleTransactions: Array.isArray(catDetails.transactions) ? catDetails.transactions.slice(0, 3) : [],
    });
    ...
    if (
      ...
      const txnLines = catDetails.transactions.map((txn) => {
        ...
        return `- ${date} | ${merchant}${name !== merchant ? ` (${name})` : ""}: $${amount.toLocaleString(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })}`;
      });
