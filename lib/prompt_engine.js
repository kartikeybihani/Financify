...
function evaluateDecisionConfidence({
  ...
  const hasRelevantMemories =
    memories.length > 0 && memories.some((m) => m.similarity > 0.5);
  ...
  if (intent_type === "actionable") {
    ...
    if (!hasBaseData) {
      ...
      clarificationQuestion =
        "I need to see your financial data to give you specific advice. Can you connect your accounts or share what you're working with?";
    } else if (basePack.liquidAssets === 0 && basePack.netWorth === 0) {
      confidenceScore = 0.4;
      needsClarification = true;
      clarificationQuestion =
        "I see your accounts are connected but I don't have current balance data. Should I work with what you have, or do you want to refresh your account data?";
    } else {
      confidenceScore = 0.8;
    }
    ...
    if (!hasBaseData) {
      ...
      needsClarification = true;
      clarificationQuestion =
        "This sounds urgent. To help you right now, I need to understand your situation. What's the most pressing issue - is it an immediate payment, overdraft risk, or something else?";
    } else {
    ...
    if (!hasBaseData && !hasRelevantMemories && intent === "ask_personalized") {
      ...
      needsClarification = true;
      clarificationQuestion =
        "Are you asking about general financial concepts, or do you want advice specific to your situation?";
    }
    ...
    if (!hasBaseData) {
      ...
      needsClarification = true;
      clarificationQuestion =
        "For long-term planning, I'll need to see your current financial picture. Do you have accounts connected, or should I help you set up a plan based on your goals?";
    } else {
  ...
  if (hasRelevantMemories) {
    const avgSimilarity =
      ...
      memories.filter((m) => m.similarity > 0.5).length;
