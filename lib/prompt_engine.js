...
function detectUserState(message, financialData = {}) {
  ...
  // Detect emotional state with confidence scores (priority order matters)
  ...
  if (panicMatches.length > 0) {
    ...
    state.confidence.emotional = Math.min(
      0.9 + panicMatches.length * 0.05,
      1.0
    );
  } else if (anxietyMatches.length > 0) {
    ...
    state.confidence.emotional = Math.min(
      0.7 + anxietyMatches.length * 0.1,
      1.0
    );
  } else if (overwhelmedMatches.length > 0) {
    ...
    state.confidence.emotional = Math.min(
      0.75 + overwhelmedMatches.length * 0.1,
      1.0
    );
  } else if (shameMatches.length > 0) {
    state.emotionalState = "ashamed";
    state.needs.push("normalization", "reassurance", "non_judgmental");
    state.detectedSignals.push("shame");
    state.confidence.emotional = Math.min(0.7 + shameMatches.length * 0.1, 1.0);
  } else if (fomoMatches.length > 0) {
    state.emotionalState = "fomo";
    state.needs.push("fomo_validation", "guilt_free_budget");
    state.detectedSignals.push("fomo");
    state.confidence.emotional = Math.min(0.65 + fomoMatches.length * 0.1, 1.0);
  }
