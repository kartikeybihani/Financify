 ...
 * Pure JS function - no LLM calls, uses heuristics
 * 
 * @param {Object} params
 * @param {string} params.intent_type - Intent type from classification
 * @param {string} params.intent - Primary intent from classification
 * @param {Object} params.basePack - Base context pack (net worth, liquidity, liabilities)
 * @param {Array} params.memories - Top memories with similarity scores
 * @returns {Object} { confidence_score, needs_clarification, clarification_question }
 */
function evaluateDecisionConfidence({ intent_type, intent, basePack, memories = [] }) {
  let confidenceScore = 0.7; // Default moderate confidence
  let needsClarification = false;
  let clarificationQuestion = null;

  // Base pack availability check
  const hasBaseData = basePack && (
    basePack.netWorth !== undefined ||
    basePack.liquidAssets !== undefined ||
    basePack.accounts?.length > 0
  );

  // Memory relevance check
  const hasRelevantMemories = memories.length > 0 && 
    memories.some(m => m.similarity > 0.5);

  // Intent type specific checks
  if (intent_type === "actionable") {
    // Actionable queries need clear financial context
    if (!hasBaseData) {
      confidenceScore = 0.3;
      needsClarification = true;
      clarificationQuestion = "I need to see your financial data to give you specific advice. Can you connect your accounts or share what you're working with?";
    } else if (basePack.liquidAssets === 0 && basePack.netWorth === 0) {
      confidenceScore = 0.4;
      needsClarification = true;
      clarificationQuestion = "I see your accounts are connected but I don't have current balance data. Should I work with what you have, or do you want to refresh your account data?";
    } else {
      confidenceScore = 0.8;
    }
  } else if (intent_type === "crisis") {
    // Crisis queries need immediate clarity
    if (!hasBaseData) {
      confidenceScore = 0.2;
      needsClarification = true;
      clarificationQuestion = "This sounds urgent. To help you right now, I need to understand your situation. What's the most pressing issue - is it an immediate payment, overdraft risk, or something else?";
    } else {
      confidenceScore = 0.9; // High confidence for crisis with data
    }
  } else if (intent_type === "exploratory") {
    // Exploratory queries can proceed with lower confidence
    confidenceScore = 0.7;
    // Only clarify if completely missing context AND no relevant memories
    if (!hasBaseData && !hasRelevantMemories && intent === "ask_personalized") {
      confidenceScore = 0.5;
      needsClarification = true;
      clarificationQuestion = "Are you asking about general financial concepts, or do you want advice specific to your situation?";
    }
  } else if (intent_type === "planning") {
    // Planning queries need financial data
    if (!hasBaseData) {
      confidenceScore = 0.3;
      needsClarification = true;
      clarificationQuestion = "For long-term planning, I'll need to see your current financial picture. Do you have accounts connected, or should I help you set up a plan based on your goals?";
    } else {
  ...
  if (intent === "goal_conversation") {
    // Goal creation needs clarity on amount/timeline
    confidenceScore = Math.max(confidenceScore, 0.6);
    // Don't clarify here - let goal flow handle it
  } else if (intent === "stock_query") {
    // Stock queries are usually clear
    confidenceScore = 0.9;
  } else if (intent === "off_topic") {
    // Off-topic doesn't need clarification
    confidenceScore = 0.95;
    needsClarification = false;
  }

  // Memory boost: Relevant memories increase confidence
  if (hasRelevantMemories) {
    const avgSimilarity = memories
      .filter(m => m.similarity > 0.5)
      .reduce((sum, m) => sum + m.similarity, 0) / memories.filter(m => m.similarity > 0.5).length;
    if (avgSimilarity > 0.7) {
      confidenceScore = Math.min(confidenceScore + 0.1, 0.95);
    }
  }

  // Final threshold: Only clarify if confidence is below threshold
 ...
 */
function buildClarificationPrompt(clarificationQuestion, style = "conversational") {
