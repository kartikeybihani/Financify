...
/**
 * Builds a context-aware clarifying question using financial data
 ...
 * @returns {string} Context-aware clarifying question
 */
function buildClarificationQuestion({
  ...
  // Build emotional acknowledgement prefix based on emotional state
  const getEmotionalAck = () => {
    switch (emotional_state) {
      case "anxious":
        return "I know this is a big decision and it can feel overwhelming. ";
      case "panicked":
        return "I hear the urgency in this. ";
      case "ashamed":
        return "No judgment here—let's figure this out together. ";
      case "overwhelmed":
        return "This feels like a lot to figure out, and that's totally valid. ";
      case "fomo":
        return "I get the impulse—let's make sure you're set up for success. ";
      case "neutral":
      default:
        return ""; // No emotional prefix for neutral
    }
  };

  // Build style-specific expression based on communication style
  const getStyleExpression = () => {
    switch (style) {
      case "witty":
        // Witty expressions - playful but brief
        return "Quick question: ";
      case "direct":
        // Direct expressions - straight to the point
        return "Need to know: ";
      case "conversational":
      default:
        // Conversational is natural, no prefix needed
        return "";
    }
  };

  const emotionalAck = getEmotionalAck();
  const styleExpr = getStyleExpression();

  // Combine emotional and style expressions
  const prefix = emotionalAck + styleExpr;

  // Build context-aware questions based on type
  switch (clarification_type) {
    case "income_replacement":
      // For quit job, freelance, business start
      if (liquidMonths !== null) {
        return `${prefix}You've got about ${liquidMonths} months of cash runway right now. If you quit, what's the plan for replacing your income—clients lined up, or starting from zero?`;
      } else if (liquidAssets > 0) {
        return `${prefix}You have ${formatCurrency(
          liquidAssets
        )} in liquid assets. What's your plan for generating income if you quit—do you have clients lined up, or will you be building from scratch?`;
      }
      return `${prefix}What's your plan for generating income if you quit—do you have clients lined up, or will you be building from scratch?`;

    case "goal_timeline":
      // For goals, decisions with missing timeline
      return `${prefix}What's your timeline—are you planning to do this immediately, in a few months, or further out?`;

    case "intent_motivation":
      // For missing reason/motivation
      return `${prefix}What's driving this decision—is it about lifestyle change, financial opportunity, or something else?`;

    case "purpose_use":
      // For investment vs personal use, property purpose
      if (
        userMessage.toLowerCase().includes("house") ||
        userMessage.toLowerCase().includes("property")
      ) {
        return `${prefix}Are you buying this for investment (renting out), personal use, or long-term appreciation?`;
      }
      return `${prefix}What's your primary use case for this—daily use, investment, or something else?`;

    case "execution_plan":
      // For missing how-to/execution details
      return `${prefix}How are you planning to execute this—do you have a specific approach in mind, or still figuring it out?`;

    case "target_amount":
      // For missing specific amounts
      return `${prefix}What's the target amount or range you're thinking about?`;

    case "location_context":
      // For relocation, location-specific decisions
      return `${prefix}What's your timeline for relocating—are you planning to move immediately, or in a few months?`;

    default:
      // Fallback for unknown types
      return `${prefix}I need a bit more context to give you the best advice. Can you share more details about your situation?`;
  }
}
...
function buildClarificationPrompt(
  ...
  return `You are Finny, a financial coach for Gen Z.
