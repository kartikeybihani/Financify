...
2. Accounts for user's lifestyle and occupation${age ? ` (age ${age})` : ""}${occupation ? ` (${occupation})` : ""}
...
- Always include "Other" as the final category (no default limit - user sets it later)
- If no transaction history: create a standard budget based on income and user profile
- Include categories based on their actual spending patterns (if available)
- Budget should total close to income (accounting for savings)
- For new users with no history: suggest common categories with reasonable defaults
...
- Limit amounts should be realistic monthly budgets
- Total of all limits should be approximately $${Math.round(income - (savingsAmount || income * 0.2)).toLocaleString()} (income minus savings)
...
- CRITICAL: "Other" must always be included as the last category in the array (no default limit - user will set it later)
