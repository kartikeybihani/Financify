 ...
 * Analyzes last 60 days of transactions, even if no new transactions were added
 * @param {string} userId - User ID
 * @param {Array} newTransactions - Array of newly added transactions (optional, used for optimization)
...
async function detectMoneyReceived(userId, newTransactions) {
  try {
    ...
    // Get all positive transactions from last 60 days
    ...
    // Check if we already notified about money received in last 7 days
    ...
    const { data: existingTriggers } = await supabase
      ...
      .select("id")
      .eq("user_id", userId)
      .in("trigger_type", ["paycheck", "money_received"])
      .eq("status", "sent")
    ...
    // Process each significant inflow (prioritize most recent)
    for (const tx of recentInflows) {
      ...
      // Prioritize recent transactions (within last 7 days)
      if (amount < 50 && daysAgo > 7) {
        continue;
      }
      
      // Lower threshold for very recent transactions (last 3 days)
      if (amount < 25 && daysAgo > 3) {
        continue;
      }
      ...
      if (trigger) {
        ...
        // Only create one trigger per sync (take the most recent significant one)
        break;
      }
    }
 ...
 * Analyzes last 60 days for better context
 */
async function detectSpendingSpikes(userId) {
  try {
    ...
    // Get average daily spending from last 60 days
    ...
    const avgDailySpending = historicalTotal / 60;
...
/**
 * LLM-based pattern detection - analyze last 60 days of transactions for creative insights
...
async function detectLLMPatterns(userId) {
  try {
    ...
    const { data: transactions, error } = await supabase
      ...
      .neq("transaction_type", "transfer")
      .order("date", { ascending: false })
      .limit(300); // Last 300 transactions (more data for better analysis)
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      console.warn("[PATTERN_DETECTION] No OPENROUTER_API_KEY, skipping LLM detection");
      return [];
    }
