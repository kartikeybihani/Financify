/**
 * Basic onboarding test script: fetch last 6 months of transactions for a user.
 *
 * Usage:
 *   node basic.js <USER_ID>
 *   USER_ID=<USER_ID> node basic.js
 *
 * Env:
 *   SUPABASE_URL / EXPO_PUBLIC_SUPABASE_URL
 *   SUPABASE_SERVICE_ROLE_KEY / EXPO_PUBLIC_SUPABASE_SERVICE_ROLE_KEY (preferred)
 *   SUPABASE_ANON_KEY / EXPO_PUBLIC_SUPABASE_ANON_KEY (fallback)
 */
...
const shouldRunLLM = process.argv.includes("--llm") || process.env.RUN_LLM === "1";
...
if (!userId) {
  console.error("Missing USER_ID. Usage: node basic.js <USER_ID>");
  process.exit(1);
}

if (!supabaseUrl || !supabaseKey) {
  console.error(
    "Missing Supabase env. Need SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY (or anon key)."
  );
  process.exit(1);
}
...
const extractFirstJsonObjectFromText = (text) => {
  const s = String(text || "");
...
const buildOnboardingPrompt = (inputJson) => {
...
const callOnboardingLLM = async ({ patterns, analysisWindow = "last 6 months" }) => {
  ...
  const inputJson = {
    user_profile: {
      ...
      finny_style: "highly_witty and clever and funny",
    },
    analysis_window: analysisWindow,
    patterns: inputPatterns,
  };

  const { system, user } = buildOnboardingPrompt(inputJson);

  // Debug: log the exact user message sent to the LLM.
  const extractJsonFromUserMessage = (s) => {
    const text = String(s || "");
    const marker = "Input data (JSON):";
  ...
  const prettyUserJson = extractJsonFromUserMessage(user);
  console.log(
    JSON.stringify(
      {
        llmUserMessage: {
          text: user,
          parsedJson: prettyUserJson,
        },
      },
      null,
      2
...
const isLikelyInternalOrPayment = (tx) => {
...
const computePatterns = ({ transactions, months }) => {
  ...
  const recentLate = dayOfMonthHistogram.slice(24).reduce((a, b) => a + b, 0);
...
const logPrettyOnboardingCopy = (maybeJson) => {
  if (!maybeJson || typeof maybeJson !== "object") return;

  const intro = typeof maybeJson.intro_line === "string" ? maybeJson.intro_line : null;
  const mirror = typeof maybeJson.mirror === "string" ? maybeJson.mirror : null;
  const plan = typeof maybeJson.plan === "string" ? maybeJson.plan : null;
  const hook = typeof maybeJson.hook === "string" ? maybeJson.hook : null;

  if (!intro && !mirror && !plan && !hook) return;

  console.log(
    [
      intro ? `intro_line: ${intro}` : null,
...
const getPatternFrequencyScore = (pattern) => {
...
(async () => {
  ...
  console.log(JSON.stringify({ userId, startDate, endDate }, null, 2));

  const all = await fetchTransactions({ startDate, endDate });
  const filtered = all.filter((tx) => !isLikelyInternalOrPayment(tx));

  console.log(JSON.stringify({ fetched: all.length, afterFiltering: filtered.length }, null, 2));

  const patternPayload = computePatterns({ transactions: filtered, months });
  // console.log(JSON.stringify({ patternPayload }, null, 2));
  ...
  if (shouldRunLLM) {
    ...
    if (llmResult?.ok && llmResult?.json) {
      logPrettyOnboardingCopy(llmResult.json);
    } else {
      ...
      if (recovered) logPrettyOnboardingCopy(recovered);
    }
