/**
 * Comprehensive Plaid Category Mapper (TypeScript/Deno version)
 ...
 */
const DETAILED_CATEGORY_MAP: Record<string, CategoryMapping> = {
  ...
  "GENERAL_MERCHANDISE_DISCOUNT_STORES": { top: "Shopping", sub: "Discount Stores" },
  ...
  "BANK_TRANSFERS_INTERNAL": { top: "INTERNAL_TRANSFER", sub: "INTERNAL_TRANSFER" },
 ...
 */
const PRIMARY_CATEGORY_MAP: Record<string, CategoryMapping> = {
  "FOOD_AND_DRINK": { top: "Food", sub: "Dining Out" },
  "GENERAL_MERCHANDISE": { top: "Shopping", sub: "Shopping" },
  "TRANSPORTATION": { top: "Transportation", sub: "Transportation" },
  "ENTERTAINMENT": { top: "Entertainment", sub: "Entertainment" },
 ...
 */
function parseSubcategory(detailed: string | null | undefined): string | null {
 ...
 */
export function mapPlaidToAppCategory(
  primary: string | null | undefined,
  detailed: string | null | undefined
): CategoryMapping {
  ...
  // Priority 2: Smart Food vs Groceries detection
  ...
  if (upperPrimary === "TRANSFER_IN" || upperPrimary === "TRANSFER_OUT" || upperPrimary === "BANK_TRANSFERS") {
  ...
  if (detailed) {
    if (upperDetailed.includes("GROCERY") || upperDetailed.includes("SUPERMARKET")) {
      return { top: "Groceries", sub: "Groceries" };
    }
    if (upperDetailed.includes("RESTAURANT") || upperDetailed.includes("FAST_FOOD") || upperDetailed.includes("COFFEE")) {
      return { top: "Food", sub: parseSubcategory(detailed) || "Dining Out" };
    }
    if (upperDetailed.includes("STREAMING") || upperDetailed.includes("TV_AND_MOVIES")) {
      return { top: "Subscriptions", sub: "Streaming" };
    }
    if (upperDetailed.includes("UTILITIES") || upperDetailed.includes("PHONE") || upperDetailed.includes("INTERNET")) {
    ...
    if (upperDetailed.includes("HEALTH") || upperDetailed.includes("MEDICAL") || upperDetailed.includes("PHARMACY")) {
      return { top: "Health", sub: parseSubcategory(detailed) || "Health" };
    }
    if (upperDetailed.includes("TRANSPORT") || upperDetailed.includes("GAS") || upperDetailed.includes("UBER") || upperDetailed.includes("LYFT")) {
      return { top: "Transportation", sub: parseSubcategory(detailed) || "Transportation" };
    }
    if (upperDetailed.includes("TRAVEL") || upperDetailed.includes("FLIGHT") || upperDetailed.includes("HOTEL")) {
      return { top: "Travel", sub: parseSubcategory(detailed) || "Travel" };
    }
    if (upperDetailed.includes("SHOPPING") || upperDetailed.includes("MERCHANDISE")) {
      return { top: "Shopping", sub: parseSubcategory(detailed) || "Shopping" };
    }
    if (upperDetailed.includes("INCOME") || upperDetailed.includes("WAGE") || upperDetailed.includes("SALARY")) {
      return { top: "Income", sub: parseSubcategory(detailed) || "Income" };
    }
    if (upperDetailed.includes("SAVINGS") || upperDetailed.includes("INVESTMENT")) {
      return { top: "Savings", sub: parseSubcategory(detailed) || "Savings" };
    }
    if (upperDetailed.includes("PERSONAL_CARE") || upperDetailed.includes("BEAUTY") || upperDetailed.includes("HAIR")) {
      return { top: "Personal Care", sub: parseSubcategory(detailed) || "Personal Care" };
    }
    if (upperDetailed.includes("EDUCATION") || upperDetailed.includes("STUDENT") || upperDetailed.includes("SCHOOL")) {
